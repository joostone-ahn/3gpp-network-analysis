"""
Work Item Network Builder Module (Stage 6)

Work Item 간 협력 네트워크 Edge list를 생성하는 모듈.

요구사항:
- 9.1: 동일 기업이 기고한 Work Item들의 모든 쌍(combination)을 Edge로 생성
- 9.2: (A,B)와 (B,A)를 동일 Edge로 정규화하고 중복 제거
- 9.3: 두 Work Item에 동시에 기고한 고유 기업 수를 Edge Weight로 계산
- 9.4: TSG 그룹(ALL/RAN/SA/CT) × 시간 단위(연도/Release/분기) × threshold 조합별 Edge list 생성
- 9.5: CompanyNetworkBuilder와 동일한 파일 네이밍 패턴으로 저장

Edge 생성 알고리즘:
- 기업별로 그룹화하여 해당 기업이 기고한 WI 목록 추출
- itertools.combinations 사용하여 WI 쌍 생성
- Source < Target 알파벳 순으로 정규화
- 동일 WI 쌍에 기고한 고유 기업 수를 Weight로 집계

파일 저장 규칙:
- 네이밍 패턴: wi_{tsg}_{time_unit}_{time_value}_{threshold}.parquet
- 예: wi_ALL_year_2023_0.parquet, wi_RAN_release_17_3.parquet
"""
...
# 모듈 로거
logger = get_logger("network", default_stage="wi_network")
...
@dataclass
class WIEdgeSchema:
    """Work Item 간 네트워크 Edge 스키마.
    
    네트워크 Edge list DataFrame의 스키마를 정의한다.
    
    Attributes:
        Source: Work Item A (알파벳 순으로 정렬, Source < Target)
        Target: Work Item B (알파벳 순으로 정렬, Source < Target)
        Weight: 두 WI에 동시에 기고한 고유 기업 수
        Companies: 동시에 기고한 기업 목록 (선택적)
    ...
    Example:
        ...
        ...     Target="NR_FR2_enh",
        ...     Weight=8,
        ...     Companies=["Samsung Electronics", "Huawei", ...]
        ... )
    """
    Source: str
    Target: str
...
class WINetworkBuilder:
    ...
    동일 기업이 기고한 Work Item들 간의 관계를 네트워크 Edge로 변환한다.
    
    Edge 생성 알고리즘:
        1. 기업별로 그룹화하여 해당 기업이 기고한 WI 목록 추출
        2. 각 기업의 WI 목록에서 모든 WI 쌍 조합(combinations) 생성
        3. (A,B)와 (B,A) 정규화: sorted([A,B]) → (Source, Target)
        4. 동일 WI 쌍에 기고한 고유 기업 수를 Weight로 집계
        5. threshold 적용하여 낮은 Weight Edge 제거
    ...
    Example:
        ...
        >>> builder = WINetworkBuilder(config)
    ...
    def _create_edges_from_company(
        self,
        work_items: List[str],
        company: str,
    ) -> List[Dict[str, Any]]:
        """단일 기업에서 WI 쌍 Edge 생성.
        
        Args:
            work_items: 기업이 기고한 WI 목록 (중복 제거됨)
            company: 기업명
        ...
        # 정렬된 WI 목록에서 모든 쌍 조합 생성
        sorted_wis = sorted(work_items)
        
        for pair in itertools.combinations(sorted_wis, 2):
    ...
    def build_edges(
    ...
    ) -> Tuple[pd.DataFrame, WINetworkBuildStats]:
        ...
        처리 흐름:
            ...
            2. 시간 단위 필터링
            3. 기업별로 그룹화
            4. 각 기업 내 고유 WI 목록 추출
            5. WI 쌍 조합 생성 (itertools.combinations)
            6. Edge 정규화 (Source < Target)
            7. Weight 집계 (동일 WI 쌍에 기고한 고유 기업 수)
            8. Threshold 적용
        ...
        Args:
            ...
            include_companies: Edge별 기업 목록 포함 여부
        ...
        if len(filtered_df) == 0:
            ...
            if include_companies:
                empty_df["Companies"] = pd.Series(dtype=object)
        ...
        # 기업별 Edge 생성 (요구사항 9.1)
        all_edges: List[Dict[str, Any]] = []
        
        for company, group in filtered_df.groupby("company"):
            # 기업 내 고유 WI 목록 (중복 제거)
            unique_wis = group["Work_Item"].unique().tolist()
            
            # WI 쌍 Edge 생성
            edges = self._create_edges_from_company(unique_wis, str(company))
        ...
        # Weight 집계 (요구사항 9.3): 동일 WI 쌍에 기고한 고유 기업 수
        if include_companies:
            # 기업 목록 포함
            weight_df = (
                edge_df.groupby(["Source", "Target"])
                .agg(
                    Weight=("Company", "nunique"),
                    Companies=("Company", lambda x: list(x.unique())),
                )
                .reset_index()
            )
        else:
        ...
        # 로그 기록
        logger.info_with_details(
            f"WI Edge 생성 완료: {tsg_group}/{time_unit}/{time_value} (threshold={threshold}): "
    ...
    def _generate_filename(
        ...
        Returns:
            파일명 문자열
        """
        return f"wi_{tsg_group}_{time_unit}_{time_value}_{threshold}.parquet"
    ...
    def build_all(
        ...
        include_companies: bool = False,
    ) -> Dict[str, Tuple[pd.DataFrame, WINetworkBuildStats]]:
        """모든 조합에 대한 WI Edge list 생성.
        ...
        results: Dict[str, Tuple[pd.DataFrame, WINetworkBuildStats]] = {}
        total_combinations = 0
        successful = 0
        
        logger.info_with_details(
            f"전체 WI Edge list 생성 시작",
            stage="wi_network",
            details={
                "tsg_groups": self.config.tsg_groups,
                "time_units": self.config.time_units,
                    ...
                    for threshold in self.config.thresholds:
                        ...
                        try:
                            ...
                            logger.error_with_details(
                                f"WI Edge 생성 실패: {tsg_group}/{time_unit}/{time_value}/{threshold}",
                                stage="wi_network",
                                details={
                                    "error": str(e),
                                    "tsg_group": tsg_group,
        ...
        logger.info_with_details(
            f"전체 WI Edge list 생성 완료: {successful}/{total_combinations} 성공",
            stage="wi_network",
            details={
    ...
    @classmethod
    def from_config_file(cls, config_path: Optional[Path] = None) -> "WINetworkBuilder":
        """설정 파일로부터 WINetworkBuilder 생성.
        
        Args:
            config_path: network.yaml 파일 경로. None이면 기본 경로 사용.
            
        Returns:
            WINetworkBuilder 인스턴스
        """
        from src.utils.config_loader import load_config
        config = load_config("network")
